            ...
            () =>
            {
                ...
                lock (_storesLock)
                {
                    if (!_options.IgnoreLocalDb)
                    {
                        ...
                        _logger.Info<PublishedSnapshotService, bool>("Creating the content store, localContentDbExists? {LocalContentDbExists}", _documentRepository != null && _documentRepository.IsPopulated());
                        _contentStore = new ContentStore(PublishedSnapshotAccessor, VariationContextAccessor, _logger, _documentRepository);
                        _logger.Info<PublishedSnapshotService, bool>("Creating the media store, localMediaDbExists? {LocalMediaDbExists}", _mediaRepository != null && _mediaRepository.IsPopulated());
